...
- GA4: 株式会社OneBeの専用プロパティ `DEMO_food1`、測定ID `G-CWWJ1LKKBL`。本番の `/sample_food1/` のみで有効にし、ローカルプレビューは除外します。
- ページ閲覧・スクロール・離脱クリックを計測。「Web制作を相談する」は `web_consultation_click` として別途記録します。これはLINEへ進むクリック数であり、相談完了や友だち追加数ではありません。
- 広告パーソナライズ、Googleシグナルをタグで無効化。予約デモのフォーム自動計測はオフで、入力内容は計測コードへ渡しません。サイト内のプライバシー説明にもGA4利用を記載しています。
